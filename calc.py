...
def popupmsg():
    ...
    popup.geometry("150x100")
    popup.title("Alert")
    label = Label(popup,text="Cannot divide by zer0! /n Enter valid value")
    label.pack(side="top",fill="x",pady=10)
    B1 = Button(popup,text="Okay",bg="#DDDDDD",command=popup.destroy)
    B1.pack()
...
root.resizable(0,0)
ef = font.Font(size=15)
entry = Entry(root,justify='right',font=ef)
entry.grid(row=0,column=0,columnspan=4,sticky=N+E+S+W,padx=5,pady=5)
...
button15 = Button(root, text='Del', fg=text_fg, bg=other_button_bg, padx=10, pady=3,
